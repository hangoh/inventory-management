...
urlpatterns = [
    path('',StoreView.as_view({"get":"retrieve", "post":"create"}), name="list_stores"),
    path('<int:store_id>',StoreView.as_view({"get":"retrieve", "put":"update", "delete":"destroy"}),name="store"),
    path("<int:store_id>/products",ProductView.as_view({"get":"retrieve", "post":"create"}),name='list_products'),
